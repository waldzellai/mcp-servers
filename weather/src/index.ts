	...
	server.tool(
		...
		async ({ location }) => {
			try {
				...
				console.log(
					`[Weather] Location: ${coords.latitude},${coords.longitude}`,
				)
				...
				console.log(
					`[Weather] Formatted local time: ${formatLocalTime(obs.timestamp, timezone)}`,
				)
			...
			try {
				...
				const filteredAlerts =
					severity === "all"
						? alerts
						: alerts.filter(
								alert => alert.properties.severity?.toLowerCase() === severity,
							)
				...
				for (const station of stations) {
					...
					try {
						...
						if (obsData.properties) {
							...
							const localObsTime = formatLocalTime(
								obs.timestamp,
								stationTimezone,
							)
	...
	server.tool(
		...
		async ({ location }) => {
			try {
				...
				console.log(
					`[LocalTime] Location: ${coords.latitude},${coords.longitude}`,
				)
